...
module RailsAdmin
  module ApplicationHelper
    ...
    def edit_user_link
      ...
      link_to _current_user.nickname, "/settings/profile"
    end
      ...
      actions.map do |action|
        ...
        %{
          <li title="#{wording if only_icon}" rel="#{'tooltip' if only_icon}" class="icon #{action.key}_#{parent}_link #{'active' if current_action?(action)}">
            <a class="#{action.key == :show_in_app ? '' : 'pjax'}" href="#{url_for({ :action => action.action_name, :controller => 'rails_admin/main', :model_name => abstract_model.try(:to_param), :id => (object.try(:persisted?) && object.try(:id) || nil) })}" target='_blank'>
